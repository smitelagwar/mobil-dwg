...
9. `docs/ARCHITECTURE.md` ve `MobilDwg.sln`
10. `docs/evidence/STAGE_04.md`
11. `docs/evidence/STAGE_03.md`, `fixtures/manifest/stage03-mini.json`, `fixtures/manifest/stage03-source-integrity.json`, `docs/GOLDEN_CONTRACT.md`, `docs/DEVICE_MATRIX.md`
12. `docs/evidence/STAGE_02.md` ve `compliance/DEPENDENCY_EVIDENCE.md`
13. `docs/EXECUTION_LOG.md`
14. `docs/TOOLCHAIN.md`, `docs/evidence/STAGE_01.md`, `docs/STAGE_01_IOS_ACCESS_INVENTORY.md`
...
STAGE09_USER_GO: GRANTED — kullanıcı custom renderer implementation başlangıcını açıkça onayladı
STAGE09_BRANCH: stage09-render-scene-camera
STAGE09_PR: #12 — OPEN / NOT_MERGED
STAGE09_SOURCE_TEST_HEAD: 9a17d333afc0a3df1de856a9a53fae0e74617c29
STAGE09_LATEST_WORKFLOW_HEAD: 0c5aa84bf491ec24c4409c35ffad83dd159b9290
STAGE09_IMPLEMENTATION: compact immutable RenderScene; stable metadata; double camera; RenderViewport bridge; precision/overflow guards; OCS/WCS; diagnostics; deterministic snapshot
STAGE09_VALIDATION: NOT_EXECUTED — hosted/self-hosted denemelerde checkout/build/test step'i başlamadı
STAGE09_UBUNTU: run 32791364379 / #30; rerun job 97690824454; steps=[]; runner_id=0
STAGE09_MACOS: run 32786600644 / #14; attempts 1/2/3; steps=[]; runner_id=0
STAGE09_UBUNTU_SLIM: run 32811281420 / #32; job 97690952636; steps=[]; runner_id=0
STAGE09_SELF_HOSTED: run 32784140351 / #3; uygun online runner atanmadı; geçici workflow silindi
...
NEXT_WORK_STATUS: IN_PROGRESS / WAITING_REAL_DOTNET_10_0_400_EXECUTION
NEXT_ACTION: exact .NET 10.0.400 çalışan bir environment elde et; T0 restore/build + T1 deterministic scene/camera testlerini çalıştır; gerçek marker/artifact olmadan PR #12 merge etme veya AŞAMA 10'a geçme.
...
AŞAMA 09 için ADR 0002'deki yüksek efor/bakım riski kullanıcı tarafından açıkça kabul edilmiştir ve implementation başlamıştır. Bu GO yeniden istenmez. Buna karşılık AŞAMA 09'un kendi T0/T1 çıkış kapısı halen zorunludur; runner allocation problemi stage'i PASS/DONE yapmaz.
...
- default value-object metadata bypass ve duplicate ID guard'ları;
- Stage 04 render-contract marker geriye uyumlu tutuldu.

Son source/test hardening head: `9a17d333afc0a3df1de856a9a53fae0e74617c29`. Son workflow/fallback head: `0c5aa84bf491ec24c4409c35ffad83dd159b9290`.

Gerçek T0/T1 henüz yürütülemedi. Standard `ubuntu-latest`, `macos-26` ve ayrı `ubuntu-slim` hosted pool'larında job'lar checkout başlamadan `steps=[]`, `runner_id=0` ile kesildi. Son bağımsız-pool kayıt: run `32811281420`/#32, job `97690952636`, label `ubuntu-slim`. Standard Ubuntu run #30 explicit rerun job `97690824454` de aynı semptomu verdi. Configured self-hosted Windows probe uygun online runner bulamadı ve geçici workflow kaldırıldı. Bu nedenle compile/test failure veya PASS iddiası yoktur.
...
- AŞAMA 09 custom scene yolu kullanıcı GO'su ile başlamıştır; T0/T1 olmadan merge/DONE yapılmaz.
...
1. Bu dosyayı, `DEVAM.md`, aktif Stage 09 evidence ve execution override'ı oku.
2. Gerçek `main` ve PR #12 head durumunu doğrula.
3. Kullanıcı değişikliklerini koru; destructive Git işlemi yapma.
4. Kullanıcı yalnız `devam` diyorsa **AŞAMA 09'dan devam et**; GO yeniden isteme.
5. Runner label değiştirerek tekrar zinciri üretme; standard Ubuntu, macOS ve ubuntu-slim allocation denemeleri tüketildi.
6. Exact .NET `10.0.400` çalışan gerçek bir execution environment varsa Stage 09 T0/T1'i çalıştır.
7. AŞAMA 01/AŞAMA 06/AŞAMA 08 dış erişim kapılarını sahte PASS/DONE yapma.
8. Bir turda en fazla bir aşama tamamla.
9. `[LIVE-VERIFY]` noktalarında resmi/güncel kaynak kullan.
10. Her turun sonunda `gecmis.md`, ilgili stage evidence ve canonical checkpoint'i gerçek durumla güncelle.

## Bir sonraki tur

Yalnız AŞAMA 09 sürdürülür. Exact .NET `10.0.400` üzerinde `tests/MobilDwg.Rendering.Tests/MobilDwg.Rendering.Tests.csproj` restore/build ve deterministic executable testleri gerçekten çalıştırılmadan Stage 09 kapatılmaz. Beklenen marker'lar: `STAGE09_DOTNET_PIN_PASS`, `STAGE09_T0_BUILD_PASS`, `STAGE04_RENDER_CONTRACT_TESTS_PASS`, `STAGE09_RENDER_SCENE_TESTS_PASS`, `STAGE09_T1_SCENE_PASS`, `render-scene/v1`. Gerçek PASS ve evidence artifact/log elde edilirse AŞAMA 09 checkpoint'i `DONE` yapılır ve PR #12 merge edilir; **aynı turda AŞAMA 10 başlatılmaz**.
