<#
.SYNOPSIS
    AŞAMA 18 real MobilDwg.App API36 Viewer UX & Lifecycle acceptance gate.
.DESCRIPTION
    Builds a validation-only Release APK with -p:A18Validation=true, executes
    recent files bounded metadata persistence, log redaction sanitization,
    viewer session initialization and ZoomToFit, Pan & Zoom viewport manipulation,
    zero-reparse layer visibility toggling, zero-reparse layout switching,
    metadata and diagnostics inspection, simulated back navigation dispatch,
    orientation resize & memory pressure handling, verifies markers and UI hierarchy,
    captures PNG evidence, and validates package-scoped crash/ANR/liveness.
#>
param(
    [string]$Configuration = 'Release',
    [string]$ArtifactsDir = 'artifacts/a18-android-viewer-lifecycle'
)
...
function Fail([string]$Message) {
    Write-Host "A18_FAIL: $Message" -ForegroundColor Red
    exit 1
}
...
Write-Host "A18_EMULATOR_API36_PASS serial=$Serial android=$AndroidRelease abi=$Abi"

$AppProject = 'src/MobilDwg.App/MobilDwg.App.csproj'
& dotnet build $AppProject -f net10.0-android36.0 -c $Configuration -p:A18Validation=true -warnaserror --nologo | Tee-Object -FilePath (Join-Path $ArtifactsFullPath 'app-build.log')
Require-ExitCode "A18 validation MobilDwg.App build"
...
if (-not $Apk) { Fail "no A18 validation APK produced" }
$ApkHash = (Get-FileHash -LiteralPath $Apk.FullName -Algorithm SHA256).Hash.ToLowerInvariant()
$EvidenceApk = Join-Path $ArtifactsFullPath 'MobilDwg.App-A18-Signed.apk'
Copy-Item -LiteralPath $Apk.FullName -Destination $EvidenceApk -Force
Write-Host "A18_REAL_APP_APK_PASS bytes=$($Apk.Length) sha256=$ApkHash"
...
Require-ExitCode "A18 adb install"
...
Write-Host "A18_REAL_APP_INSTALL_PASS package=$PackageName launcher=$Launcher"
...
if ($LaunchOutput -notmatch 'Status:\s+ok' -and $LaunchOutput -notmatch 'Status:\s+timeout') { Fail "A18 app launch failed: $LaunchOutput" }
$AppPid = ((& adb -s $Serial shell pidof -s $PackageName 2>$null) | Out-String).Trim()
if ($AppPid -notmatch '^\d+$') {
    Start-Sleep -Seconds 2
    $AppPid = ((& adb -s $Serial shell pidof -s $PackageName 2>$null) | Out-String).Trim()
}
if ($AppPid -notmatch '^\d+$') { Fail "A18 app PID not found" }
Write-Host "A18_REAL_APP_LAUNCH_PASS pid=$AppPid"

Write-Host "Waiting for A18 Viewer UX/lifecycle validation to complete..."
$sw = [System.Diagnostics.Stopwatch]::StartNew()
$markerFound = $false
while ($sw.Elapsed.TotalSeconds -lt 35) {
    $currentLog = (& adb -s $Serial logcat -d -t 300 | Out-String)
    if ($currentLog.IndexOf('A18_REAL_APP_UI_IMAGE_READY', [System.StringComparison]::Ordinal) -ge 0) {
        $markerFound = $true
        break
    }
    Start-Sleep -Seconds 2
...
if (-not $markerFound) {
    Write-Warning "A18_REAL_APP_UI_IMAGE_READY not found within 35 seconds, checking logcat..."
}
...
foreach ($marker in @(
    'A18_ANDROID_RECENT_FILES_PASS',
    'A18_ANDROID_LOG_REDACTION_PASS',
    'A18_ANDROID_SESSION_INIT_PASS',
    'A18_ANDROID_PAN_ZOOM_PASS',
    'A18_ANDROID_LAYER_TOGGLE_PASS',
    'A18_ANDROID_LAYOUT_SWITCH_PASS',
    'A18_ANDROID_INFO_DIAGNOSTICS_PASS',
    'A18_ANDROID_BACK_NAVIGATION_PASS',
    'A18_ANDROID_ORIENTATION_TRIM_PASS',
    'A18_SNAPSHOT_HASH=',
    'A18_ANDROID_SKIA_RENDER_PASS bytes=',
    'ANDROID_STAGE18_VIEWER_LIFECYCLE_PASS',
    'A18_REAL_APP_UI_IMAGE_READY sha256='
)) {
    if ($RawLogcat.IndexOf($marker, [StringComparison]::Ordinal) -lt 0) { Fail "required app marker missing: $marker" }
}
Write-Host "A18_REAL_APP_VIEWER_MARKERS_PASS"

$UiRemote = '/sdcard/a18-window.xml'
$UiLocal = Join-Path $ArtifactsFullPath 'window.xml'
& adb -s $Serial shell uiautomator dump $UiRemote | Out-Null
Require-ExitCode "A18 uiautomator dump"
& adb -s $Serial pull $UiRemote $UiLocal | Out-Null
Require-ExitCode "A18 uiautomator pull"
$UiXml = Get-Content -Raw $UiLocal
if ($UiXml.IndexOf('ANDROID_STAGE18_VIEWER_LIFECYCLE_PASS', [StringComparison]::Ordinal) -lt 0) {
    Fail "A18 validation status is not visible in the real app UI hierarchy"
}
Write-Host "A18_REAL_APP_UI_STATUS_PASS"

# Simulate memory trim
$previousEap = $ErrorActionPreference
$ErrorActionPreference = 'Continue'
& adb -s $Serial shell am send-trim-memory $PackageName COMPLETE 2>$null | Out-Null
$ErrorActionPreference = $previousEap
Start-Sleep -Milliseconds 500

$Screenshot = Join-Path $ArtifactsFullPath 'a18-real-app-viewer.png'
...
Write-Host "A18_SCREENSHOT_PNG_PASS bytes=$((Get-Item $Screenshot).Length) sha256=$ScreenshotHash"
...
if ($AppPidStillAlive -ne $AppPid) { Fail "A18 app process did not remain alive" }
Write-Host "A18_REAL_APP_STABILITY_PASS pid=$AppPid"

$HeadSha = (& git rev-parse HEAD | Out-String).Trim()
$Summary = @"
A18 VIEWER UX LIFECYCLE ANDROID SUMMARY
...
Recent files bounded persistence: PASS
Log redaction path sanitization: PASS
Session initialization & ZoomToFit: PASS
Pan & Zoom viewport control: PASS
Zero-reparse layer visibility toggle: PASS
Zero-reparse layout switching: PASS
Metadata & diagnostics extraction: PASS
Simulated back navigation dispatch: PASS
Orientation resize & memory trim: PASS
Real app UI status: PASS
Crash/ANR/liveness: PASS
Physical Android: DEFERRED_RELEASE_DEVICE_GATE
Claim limit: A18_VIEWER_LIFECYCLE_API36_ONLY_NOT_CAD_PARSE_TO_SCENE_OR_PHYSICAL_DEVICE_FIDELITY
Result: ANDROID_STAGE18_VIEWER_LIFECYCLE_PASS
"@

Set-Content -Path (Join-Path $ArtifactsFullPath 'summary.txt') -Value $Summary -Encoding utf8
Write-Host "ANDROID_STAGE18_VIEWER_LIFECYCLE_PASS"
Write-Host "CLAIM_LIMIT=A18_VIEWER_LIFECYCLE_API36_ONLY_NOT_CAD_PARSE_TO_SCENE_OR_PHYSICAL_DEVICE_FIDELITY"
exit 0
