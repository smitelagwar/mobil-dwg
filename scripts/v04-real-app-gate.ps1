<#
.SYNOPSIS
    Android validation V04 gate for the real installable MobilDwg.App shell.
.DESCRIPTION
    Runs the hardened V01 infrastructure prerequisite, then independently builds,
    installs and launches the repository's real MobilDwg.App on the pinned API 36
    emulator. This proves app-shell packaging/runtime only, not parser/render fidelity.
#>
param(
...
function Invoke-AdbBinaryToFile {
    ...
    if ($process.ExitCode -ne 0) {
        Fail "adb screencap failed: $($stderrTask.Result)"
    }
}
...
if (Test-Path $ArtifactsFullPath) { Remove-Item -Recurse -Force $ArtifactsFullPath }
New-Item -ItemType Directory -Path $ArtifactsFullPath -Force | Out-Null

Write-Host "=========================================================="
Write-Host " ANDROID VALIDATION V04 - REAL MobilDwg.App SHELL"
Write-Host "=========================================================="

# 1. Re-run the proven toolchain/emulator and executable harness prerequisite.
$InfraArtifacts = Join-Path $ArtifactsDir "v01-infrastructure-prerequisite"
& .\scripts\android-emulator-gate.ps1 -Configuration $Configuration -ArtifactsDir $InfraArtifacts -AvdName $AvdName -Headless
Require-ExitCode "V01 hardened infrastructure prerequisite"
Write-Host "V04_INFRASTRUCTURE_PREREQUISITE_PASS"
...
if (-not $AdbCommand) { Fail "adb not found after infrastructure prerequisite" }
...
if (-not $Serial) { Fail "no booted emulator found after infrastructure prerequisite" }
...
# 2. Verify exact MAUI dependency policy and real app package graph.
$Cpm = [xml](Get-Content -Raw 'Directory.Packages.props')
$mauiVersionNode = @($Cpm.Project.ItemGroup.PackageVersion) | Where-Object { $_.Include -eq 'Microsoft.Maui.Controls' }
if ($mauiVersionNode.Count -ne 1 -or $mauiVersionNode[0].Version -ne '[10.0.100]') {
...
if ($VulnerabilityJson -match '"severity"\s*:') {
    Fail "NuGet reported vulnerability metadata in the real app dependency graph"
}
...
if (-not (Test-Path $MauiNuspec)) { Fail "Microsoft.Maui.Controls 10.0.100 nuspec not found in NuGet cache" }
[xml]$NuspecXml = Get-Content -Raw $MauiNuspec
$LicenseNode = $NuspecXml.SelectSingleNode("//*[local-name()='license']")
if (-not $LicenseNode -or $LicenseNode.InnerText.Trim() -ne 'MIT') {
    Fail "Microsoft.Maui.Controls 10.0.100 license is not exact MIT"
}
$MauiNupkg = Get-ChildItem -Path $MauiPackageDir -Filter '*.nupkg' -File | Select-Object -First 1
if (-not $MauiNupkg) { Fail "Microsoft.Maui.Controls 10.0.100 nupkg missing from NuGet cache" }
...
# 3. Explicit Release build of the repository's real Android app.
...
# 4. Clean install real application and verify package/launcher identity.
...
# 5. Cold launch the real app and require a live stable process.
...
# 6. Verify visible UI belongs to the real app shell.
...
if ($WindowState -notmatch [regex]::Escape($PackageName)) {
    Fail "window manager evidence does not contain the real package name"
}
Write-Host "V04_REAL_APP_UI_PASS"

# 7. Byte-safe screenshot and crash/ANR/liveness evidence.
...
Architecture/Core/Rendering prerequisite: PASS via hardened V01 executable harness gate
Install: PASS
