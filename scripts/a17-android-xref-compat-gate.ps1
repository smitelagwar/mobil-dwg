<#
.SYNOPSIS
    AŞAMA 17 real MobilDwg.App API36 XREF / Raster / Underlay / Compatibility acceptance gate.
.DESCRIPTION
    Builds a validation-only Release APK with -p:A17Validation=true, executes
    remote URL rejection (XREF_REMOTE_NOT_SUPPORTED without network calls),
    path traversal security prevention (PATH_TRAVERSAL_PREVENTED),
    case-insensitive local raster image resolution and Skia rendering,
    unresolved XREF and missing PDF underlay placeholder generation (bounding box, diagonal cross, warning label),
    verifies markers and UI hierarchy, captures PNG evidence, and validates package-scoped crash/ANR/liveness.
#>
param(
    [string]$Configuration = "Release",
    [string]$ArtifactsDir = "artifacts/a17-android-xref-compat"
)
...
function Fail([string]$Message) {
    Write-Host "A17_FAIL: $Message" -ForegroundColor Red
    exit 1
}
...
Write-Host "A17_EMULATOR_API36_PASS serial=$Serial android=$AndroidRelease abi=$Abi"

$AppProject = 'src/MobilDwg.App/MobilDwg.App.csproj'
& dotnet build $AppProject -f net10.0-android36.0 -c $Configuration -p:A17Validation=true -warnaserror --nologo | Tee-Object -FilePath (Join-Path $ArtifactsFullPath 'app-build.log')
Require-ExitCode "A17 validation MobilDwg.App build"
...
if (-not $Apk) { Fail "no A17 validation APK produced" }
$ApkHash = (Get-FileHash -LiteralPath $Apk.FullName -Algorithm SHA256).Hash.ToLowerInvariant()
$EvidenceApk = Join-Path $ArtifactsFullPath 'MobilDwg.App-A17-Signed.apk'
Copy-Item -LiteralPath $Apk.FullName -Destination $EvidenceApk -Force
Write-Host "A17_REAL_APP_APK_PASS bytes=$($Apk.Length) sha256=$ApkHash"
...
Require-ExitCode "A17 adb install"
...
Write-Host "A17_REAL_APP_INSTALL_PASS package=$PackageName launcher=$Launcher"
...
if ($LaunchOutput -notmatch 'Status:\s+ok') { Fail "A17 app launch did not report Status: ok" }
$AppPid = ((& adb -s $Serial shell pidof -s $PackageName 2>$null) | Out-String).Trim()
if ($AppPid -notmatch '^\d+$') {
    Start-Sleep -Seconds 2
    $AppPid = ((& adb -s $Serial shell pidof -s $PackageName 2>$null) | Out-String).Trim()
}
if ($AppPid -notmatch '^\d+$') { Fail "A17 app PID not found" }
Write-Host "A17_REAL_APP_LAUNCH_PASS pid=$AppPid"

Write-Host "Waiting for A17 XREF/raster/underlay validation to complete..."
$sw = [System.Diagnostics.Stopwatch]::StartNew()
$markerFound = $false
while ($sw.Elapsed.TotalSeconds -lt 35) {
    $currentLog = (& adb -s $Serial logcat -d -t 300 | Out-String)
    if ($currentLog.IndexOf('A17_REAL_APP_UI_IMAGE_READY', [System.StringComparison]::Ordinal) -ge 0) {
        $markerFound = $true
        break
    }
    Start-Sleep -Seconds 2
...
if (-not $markerFound) {
    Write-Warning "A17_REAL_APP_UI_IMAGE_READY not found within 35 seconds, checking logcat..."
}
...
foreach ($marker in @(
    ...
    'A17_ANDROID_RASTER_RESOLVED_PASS',
    'A17_ANDROID_MISSING_PLACEHOLDER_PASS',
    'A17_SCENE_ENTITIES_COUNT=',
    'A17_RENDER_PIXELS=',
    'A17_SNAPSHOT_HASH=',
    'A17_ANDROID_SKIA_RENDER_PASS bytes=',
    'ANDROID_STAGE17_XREF_COMPAT_PASS',
    'A17_REAL_APP_UI_IMAGE_READY sha256='
)) {
    if ($RawLogcat.IndexOf($marker, [StringComparison]::Ordinal) -lt 0) { Fail "required app marker missing: $marker" }
}
Write-Host "A17_REAL_APP_XREF_COMPAT_MARKERS_PASS"

$UiRemote = '/sdcard/a17-window.xml'
$UiLocal = Join-Path $ArtifactsFullPath 'window.xml'
& adb -s $Serial shell uiautomator dump $UiRemote | Out-Null
Require-ExitCode "A17 uiautomator dump"
& adb -s $Serial pull $UiRemote $UiLocal | Out-Null
Require-ExitCode "A17 uiautomator pull"
$UiXml = Get-Content -Raw $UiLocal
if ($UiXml.IndexOf('ANDROID_STAGE17_XREF_COMPAT_PASS', [StringComparison]::Ordinal) -lt 0) {
    Fail "A17 validation status is not visible in the real app UI hierarchy"
}
Write-Host "A17_REAL_APP_UI_STATUS_PASS"

$Screenshot = Join-Path $ArtifactsFullPath 'a17-real-app-xref.png'
...
Write-Host "A17_SCREENSHOT_PNG_PASS bytes=$((Get-Item $Screenshot).Length) sha256=$ScreenshotHash"
...
if ($AppPidStillAlive -ne $AppPid) { Fail "A17 app process did not remain alive" }
Write-Host "A17_REAL_APP_STABILITY_PASS pid=$AppPid"

$HeadSha = (& git rev-parse HEAD | Out-String).Trim()
$Summary = @"
A17 XREF RASTER UNDERLAY COMPATIBILITY ANDROID SUMMARY
...
Remote URL reference rejection: PASS
Path traversal prevention: PASS
Case-insensitive local raster image resolution: PASS
Raster image Skia rendering: PASS
Unresolved DWG XREF placeholder generation: PASS
Missing PDF underlay placeholder generation: PASS
Real app UI status: PASS
Crash/ANR/liveness: PASS
Physical Android: DEFERRED_RELEASE_DEVICE_GATE
Claim limit: A17_XREF_COMPAT_API36_ONLY_NOT_CAD_PARSE_TO_SCENE_OR_PHYSICAL_DEVICE_FIDELITY
Result: ANDROID_STAGE17_XREF_COMPAT_PASS
"@

Set-Content -Path (Join-Path $ArtifactsFullPath 'summary.txt') -Value $Summary -Encoding utf8
Write-Host "ANDROID_STAGE17_XREF_COMPAT_PASS"
Write-Host "CLAIM_LIMIT=A17_XREF_COMPAT_API36_ONLY_NOT_CAD_PARSE_TO_SCENE_OR_PHYSICAL_DEVICE_FIDELITY"
exit 0
