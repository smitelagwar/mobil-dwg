<#
.SYNOPSIS
    AŞAMA 12 real MobilDwg.App API36 block and insert acceptance gate.
.DESCRIPTION
    Builds a validation-only Release APK with -p:A12Validation=true, executes
    nested block transforms, non-uniform scale, mirror, Layer 0 and ByBlock inheritance,
    attributes (ATTRIB), cycle guard, depth guard, and budget guard inside the real app process,
    verifies markers and UI hierarchy, captures PNG evidence, and validates package-scoped crash/ANR/liveness.
#>
param(
    [string]$Configuration = "Release",
    [string]$ArtifactsDir = "artifacts/a12-android-block"
)
...
function Fail([string]$Message) {
    Write-Host "A12_FAIL: $Message" -ForegroundColor Red
    exit 1
}
...
Write-Host "A12_EMULATOR_API36_PASS serial=$Serial android=$AndroidRelease abi=$Abi"

$AppProject = 'src/MobilDwg.App/MobilDwg.App.csproj'
& dotnet build $AppProject -f net10.0-android36.0 -c $Configuration -p:A12Validation=true -warnaserror --nologo | Tee-Object -FilePath (Join-Path $ArtifactsFullPath 'app-build.log')
Require-ExitCode "A12 validation MobilDwg.App build"
...
if (-not $Apk) { Fail "no A12 validation APK produced" }
$ApkHash = (Get-FileHash -LiteralPath $Apk.FullName -Algorithm SHA256).Hash.ToLowerInvariant()
$EvidenceApk = Join-Path $ArtifactsFullPath 'MobilDwg.App-A12-Signed.apk'
Copy-Item -LiteralPath $Apk.FullName -Destination $EvidenceApk -Force
Write-Host "A12_REAL_APP_APK_PASS bytes=$($Apk.Length) sha256=$ApkHash"
...
Require-ExitCode "A12 adb install"
...
Write-Host "A12_REAL_APP_INSTALL_PASS package=$PackageName launcher=$Launcher"
...
if ($LaunchOutput -notmatch 'Status:\s+ok') { Fail "A12 app launch did not report Status: ok" }
$AppPid = ((& adb -s $Serial shell pidof -s $PackageName 2>$null) | Out-String).Trim()
if ($AppPid -notmatch '^\d+$') {
    Start-Sleep -Seconds 2
    $AppPid = ((& adb -s $Serial shell pidof -s $PackageName 2>$null) | Out-String).Trim()
}
if ($AppPid -notmatch '^\d+$') { Fail "A12 app PID not found" }
Write-Host "A12_REAL_APP_LAUNCH_PASS pid=$AppPid"

Write-Host "Waiting for A12 block validation to complete..."
$sw = [System.Diagnostics.Stopwatch]::StartNew()
$markerFound = $false
while ($sw.Elapsed.TotalSeconds -lt 35) {
    $currentLog = (& adb -s $Serial logcat -d -t 300 | Out-String)
    if ($currentLog.IndexOf('A12_REAL_APP_UI_IMAGE_READY', [System.StringComparison]::Ordinal) -ge 0) {
        $markerFound = $true
        break
    }
    Start-Sleep -Seconds 2
...
if (-not $markerFound) {
    Write-Warning "A12_REAL_APP_UI_IMAGE_READY not found within 35 seconds, checking logcat..."
}
...
foreach ($marker in @(
    ...
    'A12_ANDROID_PNG_PASS bytes=',
    'ANDROID_STAGE12_BLOCK_INSERT_PASS',
    'A12_REAL_APP_UI_IMAGE_READY sha256=',
...
Write-Host "A12_REAL_APP_BLOCK_MARKERS_PASS"

$UiRemote = '/sdcard/a12-window.xml'
$UiLocal = Join-Path $ArtifactsFullPath 'window.xml'
& adb -s $Serial shell uiautomator dump $UiRemote | Out-Null
Require-ExitCode "A12 uiautomator dump"
& adb -s $Serial pull $UiRemote $UiLocal | Out-Null
Require-ExitCode "A12 uiautomator pull"
$UiXml = Get-Content -Raw $UiLocal
if ($UiXml.IndexOf('ANDROID_STAGE12_BLOCK_INSERT_PASS', [StringComparison]::Ordinal) -lt 0) {
    Fail "A12 validation status is not visible in the real app UI hierarchy"
}
Write-Host "A12_REAL_APP_UI_STATUS_PASS"

$Screenshot = Join-Path $ArtifactsFullPath 'a12-real-app-block.png'
...
Write-Host "A12_SCREENSHOT_PNG_PASS bytes=$((Get-Item $Screenshot).Length) sha256=$ScreenshotHash"
...
if ($AppPidStillAlive -ne $AppPid) { Fail "A12 app process did not remain alive" }
Write-Host "A12_REAL_APP_STABILITY_PASS pid=$AppPid"

$HeadSha = (& git rev-parse HEAD | Out-String).Trim()
$Summary = @"
A12 BLOCK INSERT ATTRIBUTE ANDROID SUMMARY
...
Nested transform expansion: PASS
Non-uniform scale & mirror: PASS
Layer 0 & ByBlock inheritance: PASS
Attributes (ATTRIB) resolution: PASS
Cycle, depth, budget guards: PASS
Real app UI status: PASS
Crash/ANR/liveness: PASS
Physical Android: DEFERRED_RELEASE_DEVICE_GATE
Claim limit: A12_BLOCK_INSERT_API36_ONLY_NOT_CAD_PARSE_TO_SCENE_OR_PHYSICAL_DEVICE_FIDELITY
Result: ANDROID_STAGE12_BLOCK_INSERT_PASS
"@

Set-Content -Path (Join-Path $ArtifactsFullPath 'summary.txt') -Value $Summary -Encoding utf8
Write-Host "ANDROID_STAGE12_BLOCK_INSERT_PASS"
Write-Host "CLAIM_LIMIT=A12_BLOCK_INSERT_API36_ONLY_NOT_CAD_PARSE_TO_SCENE_OR_PHYSICAL_DEVICE_FIDELITY"
exit 0
